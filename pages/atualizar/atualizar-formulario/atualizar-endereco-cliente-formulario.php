<?php
    ...
    $endereco = $select->select("SELECT * FROM tbCliente INNER JOIN tbEndereco ON tbCliente.Cep = tbEndereco.Cep
            INNER JOIN tbCidade ON tbEndereco.CidadeId = tbCidade.CidadeId INNER JOIN tbEstado ON tbCidade.EstadoId = tbEstado.EstadoId
            WHERE ClienteId = :ClienteId", ['ClienteId' => aes_desencriptar($_GET['id'])])[0];
...
<body>
    ...
    <main>
        ...
        <section>
            <article>
                <div class="box">
                    <form action="submit/endereco-cliente-submit-update.php" method="POST">
                        <fieldset>
                            <legend><strong>Atualizar Endereços de Clientes</strong></legend>
                            <input type="hidden" name="ClienteId" value="<?= aes_encriptar($endereco['ClienteId']) ?>">
                            <div class="inputBox">
                                <input type="text" name="NomeCliente" id="NomeCliente" class="inputUser" maxlength="100" value="<?= $endereco['Nome'] ?>" readonly>
                                <label for="NomeCliente" class="labelInputValido">Nome</label>
                            </div>
                            <div class="inputBox">
                                <input type="text" name="CpfCliente" id="CpfCliente" class="inputUser" maxlength="14" onKeyPress="MascaraGenerica(this, 'CPF');" value="<?= $endereco['Cpf'] ?>" readonly valid>
                                <label for="CpfCliente" class="labelInputValido">CPF</label>
                            </div>
                            <div class="inputBox">
                                <input type="tel" name="TelefoneCliente" id="TelefoneCliente" class="inputUser" maxlength="15" onKeyPress="MascaraGenerica(this, 'TELEFONE');" value="<?= $endereco['Telefone'] ?>" readonly>
                                <label for="TelefoneCliente" class="labelInputValido">Número de Telefone</label>
                            </div>
                            <div class="inputBox">
                                <label for="DataNascimentoCliente">Data de Nascimento</label>
                                <input type="date" name="DataNascimentoCliente" id="DataNascimentoCliente" class="campoespecial" value="<?= $endereco['DataNascimento'] ?>" readonly>
                            </div>

                            <legend><strong>Endereço</strong></legend>

                            <div class="inputBox">
                                <label for="CepCliente">CEP, Logradouro, Cidade e Estado</label>
                                <select name="CepCliente" id="CepCliente" class="campoespecial">
                            ...
                            <div class="inputBox">
                                <input type="number" name="NumeroEnderecoCliente" id="NumeroEnderecoCliente" class="inputUser" min="0" max="65535" onKeyPress="MascaraInteiro(this);" value="<?= $endereco['NumeroEndereco'] ?>" required>
                                <label for="NumeroEnderecoCliente" class="labelInput">Número de Endereço</label>
                            </div>
